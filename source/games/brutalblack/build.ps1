...
# also pick each split's boss(es) (defeating one raises the level cap): gym leader(s) — any of
# the Striaton trio counts — else a name-matching boss, else the split's last fight.
$splitBoss = @{}
...
foreach ($a in $areas) {
  $ti = 0; foreach ($tr in $a.trainers) {
    ...
    if ($tr.splitAt) {
      ...
      $core = Norm ($tr.name -replace '\(.*$','')   # ignore parenthetical asides
...
# level caps per split, in story order, with the boss(es) whose defeat raises the cap
